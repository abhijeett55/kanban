...
pub fn create_configuration_file() -> Result<()> {
    ...
    println!("Wrote the default configuration file at ~/.config/kanban.toml");
    Ok(())
}

pub fn show_board(config_path: PathBuf, json: bool) -> Result<()> {
    let config = config::read_config(&config_path);
    let expanded_path = shellexpand::tilde(&config.kanban_data);
    ...
    if json {
        ...
        let done_header = format!("Done (max {})", config.done_limit);
        let mut table = Table::new();
        let columns = get_columns(tasks, config.done_limit);
        table
            .load_preset(UTF8_FULL)
            .set_content_arrangement(ContentArrangement::Dynamic)
            .set_header(vec![
                Cell::new("ToDo").fg(Color::Yellow),
                Cell::new("In-Progress (max 3)").fg(Color::Green),
                Cell::new(done_header).fg(Color::Red),
            ])
            .add_row(vec![columns.0, columns.1, columns.2]);
...
pub fn promote_task(config_path: PathBuf, id: u32) -> Result<()> {
    ...
    let ip_count = in_progress_count(tasks.clone());
    ...
    let new_state = match task.state.as_str() {
        "todo" => {
            if ip_count >= 3 {
                println!(
                    "Can not promote {}. Already three tasks In-Progress - Kanban principle.",
                    id
                );
                process::exit(1);
            } else {
...
pub fn regress_task(config_path: PathBuf, id: u32) -> Result<()> {
    ...
    let new_state = match task.state.as_str() {
        ...
        "done" => {
            ...
            if ip_count >= 3 {
                println!(
                    "Can not regress {}.  Already three items In-Progress - Kanban principle.",
                    id
                );
                process::exit(1);
            } else {
...
pub fn in_progress_count(tasks: Vec<Task>) -> usize {
    let columns = get_columns(tasks, 0);
    columns.1.split("\n").count()
}